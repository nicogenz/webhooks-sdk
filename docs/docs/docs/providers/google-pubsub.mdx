...
description: Verify Pub/Sub push deliveries — and Gmail push, Play RTDN, and Workspace Events with them — envelope unwrapped for you.
---

One provider covers everything Google delivers through Pub/Sub **push** —
Gmail push, Google Play RTDN, Workspace Events, and your own topics.
Configure the push subscription's audience and its invoker service account:
...
Each delivery carries an OIDC JWT [verified against Google's rotating
keys](/docs/providers#scheme-families) — issuer, audience, expiry, and
service account all checked.
