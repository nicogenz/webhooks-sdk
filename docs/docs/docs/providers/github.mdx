---
title: GitHub
description: Verify and handle GitHub webhooks — repository, organization, and App hooks, with replay protection one option away.
---

Set a secret on the hook, pass it to the provider, and handle events by
name — repository, organization, and GitHub App webhooks all work the same
way:
...
GitHub [signs the body alone](/docs/providers#scheme-families): the signature
proves *who* sent the request but not *when*, so the same bytes replayed
later verify perfectly. Add an
