---
title: Stripe
description: Verify and handle Stripe webhooks — replay-safe out of the box, with zero-downtime secret rotation.
---

Create the endpoint in the Stripe dashboard, pass its signing secret, and
...
Deliveries are [signed over the timestamp and body
together](/docs/providers#scheme-families), so a replayed request is rejected
on its own — you don't need anything extra for replay protection.
